...
class CustomerFeedbackAgent:
    ...
    def analyze_dataset(
        ...
        t0 = time.time()
        try:
            df_clean = sentiment_tool.run(df_clean, feedback_col, rating_col if has_rating else None)
            duration = int((time.time() - t0) * 1000)
        ...
        t0 = time.time()
        try:
            df_clean = topic_tool.run(df_clean, feedback_col, topic_tool.detect_category_column(df_clean, feedback_col))
            duration = int((time.time() - t0) * 1000)
